...
# --- СИСТЕМҮҮДИЙН Тохиргоо ---
st.set_page_config(page_title="Bene Inventory Pro", layout="wide")

HISTORY_FILE = "inventory_history.json"  # Ганцхан нэгдсэн бааз файл ашиглана
...
# Барааны нэрээр бүлэг тодорхойлох ухаалаг функц (Зөрүү хаах зориулалттай)
...
# =====================================================================
with tab1:
    st.subheader("📝 Өдрийн тооллого тулгах")
...
# =====================================================================
with tab2:
    st.subheader("📊 Сарын архив ба Ухаалаг суутгал")
    history = load_json()
    ...
    if history:
        ...
        if all_recs:
            ...
            def determine_deduction(row):
                ...
                # Хэрэв бараа өөрөө хасах боловч бүлгийн нийлбэр нь 0 буюу нэмэх байвал -> Андуурч тоолсон байна.
            ...
            if not final_deduct_df.empty:
                ...
                st.caption("ℹ️ Санамж: Панини, Бялуу зэрэг ижил төрлийн бараанууд хоорондоо зөрж (илүү, дутуу гарч) цаанаа хаагдсан бол дээрх суутгалын жагсаалтад ороогүй болно.")
            else:
            ...
            st.download_button("📥 EXCEL ТАЙЛАН ТАТАХ (Суутгал баганатай)", buffer.getvalue(), f"Uhaalag_Tailan_{start_str}_{end_str}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
